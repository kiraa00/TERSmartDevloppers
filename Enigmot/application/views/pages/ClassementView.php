<section class="banner_area">
    <div class="banner_inner d-flex align-items-center" style="min-height: 100vh">
        <div class="overlay bg-parallax" data-stellar-ratio="0.9" data-stellar-vertical-offset="0" data-background=""></div>
        <div class="container" style="margin-bottom:50px;">
            <div class="banner_content text-center p-5" style="margin-top:50px;">
                <h2 style="text-transform: none;">Classement Joueur</h2>
            </div>
            <div class="col-md-12">
                <div class="table-responsive col-lg-12 p-4" style="background-color: #5753967a;color:white;border-radius:10px;">
                    <table id="table" class="table table-striped table-bordered " cellspacing="0" style="width: 100%;">
                    <thead class="">
                        <tr>  
                            <th scope="col">#</th>  
                            <th scope="col">Pseudo</th>
                            <th scope="col">Crédit</th>  
                            <th scope="col">Points gagnés</th> 
                            <th scope="col">Phrase Créées</th>  
                            <th scope="col">Partie jouées</th>  
                        </tr> 
                    </thead>
                    <tbody>
                    </tbody>
                    <tfoot>
                        <tr>
                            <th scope="col">#</th>  
                            <th scope="col">Pseudo</th>
                            <th scope="col">Crédit</th>  
                            <th scope="col">Points gagnés</th> 
                            <th scope="col">Phrase Créées</th>  
                            <th scope="col">Partie jouées</th> 
                        </tr>
                    </tfoot>
                    </table> 
                
                </div>    
			</div>

            
        </div>
    </div>
</section>