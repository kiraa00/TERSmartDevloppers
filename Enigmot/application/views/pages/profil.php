
<section class="banner_area">
    <div class="banner_inner d-flex align-items-center" style="min-height: 100vh">
        <div class="overlay bg-parallax" data-stellar-ratio="0.9" data-stellar-vertical-offset="0" data-background=""></div>
        <div class="container">
            <div class="banner_content text-center p-5" style="margin-top:50px;">
                <h2 style="text-transform: none;">Profil Med</h2>
            </div>
            <div class="col-md-12">
                <div class="col-lg-12 p-4" style="background-color: #5753967a;">
                    <div class="row" >
                        <div class="col-md-12">
                            <div class="container-fluid">
                                <div class="rowdiv">
                                    <ul class="ul-inline">
                                        <li  ><a class="noactive" href="#">Profil</a></li>
                                        <li  ><a class="noactive" href="#">Editer mot de passe</a></li>
                                        
                                    </ul>
                                </div>
                                <br>
                                <div class="col-lg-12 p-4" style="background-color: #5753967a; margin-top:-18px;">
                                    <div class="row">
                                        <div class="col-md-6 text-left">
                                            <p><b>Parties : </b> 0 / 481</p>
                                            <p><b>Points :</b> 0 (mois : 0 / semaine : 0)</p>
                                            <p><b>Crédits :</b> 0</p>
                                               <p> Vous avez créé :</p>
                                            <ul>
                                                <li>0 phrases
                                                    <a href="/classement/personnel" style="color:white;">(Voir)</a>
                                                </li>
                                                <li>0 gloses</li>
                                                <li>0 mots ambigus</li>
                                            </ul>
                                        </div>
                                        <div class="col-md-6 text-right">
                                            <p><b>Date d'inscription :</b> 16/04/2020 à 20:44</p>
                                            <p><b>Dernière connexion :</b> 22/04/2020 à 16:51</p>
                                            <p><b>Email :</b> demahom1998@gmail.com</p>
                                            <p>
                                                <b>Groupe :</b> Membre
                                            </p>
                                            <p><b>Genre :</b> Homme</p>
                                            <p><b>Date de naissance :</b> 17/10/1907</p>
                                            
                                        </div>
                                    </div>
                                </div>
                            </div> 
                        </div>  
                    </div> 

                    

                </div>    
			</div>

            
        </div>
    </div>
</section>