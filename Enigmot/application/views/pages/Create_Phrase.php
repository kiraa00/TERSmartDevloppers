				...
				<div class="container">
					...
					<div class="col-lg-12 p-4" style="background-color: #5753967a;">
						...
						<?php if($_SESSION["user"]["credit"] < 100) { ?>
							<div style="margin-top: 15px; font-size: 13px;" class="alert alert-danger" role="alert">Vous n'avez pas assez de crédits pour pouvoir créer une phrase.</div>
						<?php } else { ?>
							<div style="margin-top: 15px; font-size: 13px;" class="alert alert-success" role="alert">Pour chaque mot ambigu ajouté, cela vous coûtera 50 crédits.<br>Pour chaque glose sélectionner dans les listes déroulante, cela vous coûtera 50 crédits.</div>
						<?php } ?>
						...
						<button id="Mot_butt" style="color: white;" class="btn btn-warning" type="button">Rendre le mot sélectionné ambigu</button>
						<div style="margin-top: 15px;" id="Mots_space" ></div>
						<button id="btnAjouter" onclick="sendData()" class="btn btn-info" type="submit">Créer la phrase</button>
					</div>
				</div>
			</div>
		</section>
